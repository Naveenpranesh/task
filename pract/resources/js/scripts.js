		...
		on_click: function () {
			...
			$(document).on("click", "button", function (ev) {
				...
				if ($(ev.target).is("button")) {
					...
					incoming_id = ev.target.getAttribute("id").trim();
					switch (incoming_id) {
						case 'get_All':
							getTable();
							break;
						...
						case "asc_arraylen_submit":
							var asc_arr_length = $("#asc_array_length").val();
							console.log(asc_arr_length)
							var html = '';
							var i = 0;
							for (i; i < asc_arr_length; i++) {
								html += '<label>Enter the index of ' + i + ':</label>';
								html += '<input type="text" id=index' + i + ' autocomplete="off"><label>Enter the value of array index ' + i + ':</label>'
								html += '<input type="number" id=' + i + ' autocomplete="off"><br>';
							...
							$(html).appendTo('#asc_array_values');

							break;

						case 'asc_proceed':

							var arr_length = $("#asc_array_length").val();

							var insert = $("input[name='asc_insert']").val();
							var _delete = $("input[name='asc_delete']").val();
							var num_delete = $("input[name='asc_num_delete']").val();
							var search = $("input[name='asc_search']").val();

							var arr = {};
							var j = 0;

							for (j; j < arr_length; j++) {
								arr[$("#index" + j).val()] = $("#" + j).val();

							}
							console.log(arr);
							var json = JSON.stringify(arr)

							var data = "arr=" + json + "&insert=" + insert + "&_delete=" + _delete + "&num_delete=" + num_delete + "&search=" + search;

							console.log(data);


							$.ajax(mtsoc_vars.base_url + 'gateway/action?application=ascbasics&action=get_results', {
								...
								$('#created').html(JSON.stringify(res['created_array']));
								...
								$('#afterInsert').html(JSON.stringify(res['inserted_array']));

								$('#afterDelete').html(JSON.stringify(res['deleted_array']));

								$('#copy').html(JSON.stringify(res['copy_array']));

								$('#merged').html(JSON.stringify(res['merged_array']));

								$('#removal').html(JSON.stringify(res['num_deleted_array']));

								$('#inilength').html(JSON.stringify(res['ini_count']));

								$('#mergedlength').html(JSON.stringify(res['mer_count']));

								$('#fiveinsert').html(JSON.stringify(res['ele_insert']));

								$('#displayfour').html(JSON.stringify(res['first_four']));


								$('#index').html(JSON.stringify(res['search']));

								// $('#unique').html(JSON.stringify(res['unique']));
