 ...
 * @filetype:	PHP
 * @filedesc:   This file is used for creating Database Query to insert,update, and select.
 * 
 *
 */
...
class QueryConstruct{
	...
	/**
	 * generate  query for db
	 * @param unknown_type $obj bean class object
	 * @param unknown_type $where condition
	 * @return string|boolean
	 */
	/*public static function Query($obj,$where=null){
		//log_twm(TWM_LOGLEVEL_INFO, "executing ".__METHOD__);
		if(is_object($obj)){
			// generate sql query for deletion
			$query = " FROM ".get_class($obj);
			if(!is_null($where) and is_string($where))	//where is set
				$query .= " WHERE $where";
			$query .= ";";
			return $query;
		}else{	
			$errmsg = " object doesn't exist";		
		}
	...
	public static function Query($obj,$where=null){
		...
		if(method_exists($obj,'getData')){
			...
			if(count($data)>0){
				$classname = get_class($obj);
				// generate sql query for updation
				$query = "UPDATE $classname SET status='d', date=CURRENT_TIMESTAMP()";
				if(!is_null($where) and is_string($where))	//where is set
					$query .= " WHERE $where";
				$query .= ";";
				return $query;
